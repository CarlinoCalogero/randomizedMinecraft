...
public class CustomAnvilMenu extends ItemCombinerMenu {
	...
	public CustomAnvilMenu(int id, Inventory inventory, ContainerLevelAccess containerLevelAccess) {
		super(ModMenuTypes.CUSTOM_ANVIL_MENU.get(), id, inventory, containerLevelAccess);
	...
	protected boolean mayPickup(Player player, boolean flag) {
		...
		System.out.println("[1]: " + (this.currentPlayerCurrency >= this.cost.get()) + " cost: " + this.cost.get()
				+ " player currency: " + this.currentPlayerCurrency);
		System.out
				.println("[2]: " + ((player.getAbilities().instabuild || this.currentPlayerCurrency >= this.cost.get())
						&& this.cost.get() > 0));
		return (player.getAbilities().instabuild || this.currentPlayerCurrency >= this.cost.get())
				&& this.cost.get() > 0;
	}

	@Override
	protected void onTake(Player inputPlayer, ItemStack inputStackItem) {
		if (!inputPlayer.getAbilities().instabuild) {
			ModMessages.sendToServer(new CurrencyManagementC2SPacket(-this.cost.get()));
		}

		float breakChance = net.nerdshelf.randomizedminecraft.event.ModEvents.ForgeEvents.customOnAnvilRepair(
				inputPlayer, inputStackItem, CustomAnvilMenu.this.inputSlots.getItem(0),
				CustomAnvilMenu.this.inputSlots.getItem(1));
		...
		this.access.execute((p_150479_, p_150480_) -> {
			...
			if (!inputPlayer.getAbilities().instabuild && blockstate.is(BlockTags.ANVIL)
					&& inputPlayer.getRandom().nextFloat() < breakChance) {
	...
	public void createResult() {
		...
		if (itemstack.isEmpty()) {
			...
			if (!net.nerdshelf.randomizedminecraft.event.ModEvents.ForgeEvents.customOnAnvilChange(this, itemstack,
					itemstack2, resultSlots, itemName, j, this.player))
				return;
			...
			this.cost.set(computeAmountToBePayed(j + i));
			if (i <= 0) {
				itemstack1 = ItemStack.EMPTY;
			}
	...
	public int computeAmountToBePayed(int vanillaAmount) {
		return vanillaAmount * 10;
	}
	...
	public boolean stillValid(Player player) {
		return stillValid(this.access, player, ModBlocks.CUSTOM_ANVIL_BLOCK.get());
	}

	/* MY METHODS */

	protected void getPlayerCurrency(Player player) {
		player.getCapability(PlayerCurrencyProvider.PLAYER_CURRENCY).ifPresent(currency -> {
			storeCurrency(currency.getCurrency());
			System.out.println(currency.getCurrency());
		});
	}

	protected void storeCurrency(int currency) {
