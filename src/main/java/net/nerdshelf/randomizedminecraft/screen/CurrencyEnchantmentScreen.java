...
@OnlyIn(Dist.CLIENT)
public class CurrencyEnchantmentScreen extends AbstractContainerScreen<CurrencyEnchantmentMenu> {
	private static final ResourceLocation ENCHANTING_TABLE_LOCATION = new ResourceLocation(
			"textures/gui/container/enchanting_table.png");
	private static final ResourceLocation ENCHANTING_BOOK_LOCATION = new ResourceLocation(
	...
	public CurrencyEnchantmentScreen(CurrencyEnchantmentMenu p_98754_, Inventory p_98755_, Component p_98756_) {
	...
	public boolean mouseClicked(double p_98758_, double p_98759_, int p_98760_) {
		...
		for (int k = 0; k < 3; ++k) {
			...
			if (d0 >= 0.0D && d1 >= 0.0D && d0 < 108.0D && d1 < 19.0D
					&& this.menu.clickMenuButton(this.minecraft.player, k)) {
	...
	protected void renderBg(PoseStack p_98762_, float p_98763_, int p_98764_, int p_98765_) {
		...
		Matrix4f matrix4f = (new Matrix4f()).translation(-0.34F, 0.23F, 0.0F).perspective(((float) Math.PI / 2F),
				1.3333334F, 9.0F, 80.0F);
		...
		VertexConsumer vertexconsumer = multibuffersource$buffersource
				.getBuffer(this.bookModel.renderType(ENCHANTING_BOOK_LOCATION));
		this.bookModel.renderToBuffer(p_98762_, vertexconsumer, 15728880, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F,
				1.0F);
		...
		for (int i1 = 0; i1 < 3; ++i1) {
			...
			if (l1 == 0) {
				...
				if (((l < i1 + 1 || this.minecraft.player.experienceLevel < l1)
						&& !this.minecraft.player.getAbilities().instabuild) || this.menu.enchantClue[i1] == -1) { // Forge:
																													// render
																													// buttons
																													// as
																													// disabled
				...
				this.font.drawShadow(p_98762_, s, (float) (k1 + 86 - this.font.width(s)),
						(float) (j + 16 + 19 * i1 + 7), j2);
			}
		}
		...
		for (int j = 0; j < 3; ++j) {
			...
			if (this.isHovering(60, 14 + 19 * j, 108, 17, (double) p_98768_, (double) p_98769_) && k > 0) {
				...
				list.add((Component.translatable("container.enchant.clue",
						enchantment == null ? "" : enchantment.getFullname(l))).withStyle(ChatFormatting.WHITE));
				if (enchantment == null) {
					list.add(Component.literal(""));
					list.add(Component.translatable("forge.container.enchant.limitedEnchantability")
							.withStyle(ChatFormatting.RED));
				} else if (!flag) {
					list.add(CommonComponents.EMPTY);
					if (this.minecraft.player.experienceLevel < k) {
						list.add(Component.translatable("randomizedminecraftmod.container.enchant.currency.requirement",
						...
						if (i1 == 1) {
							mutablecomponent1 = Component
									.translatable("randomizedminecraftmod.container.enchant.currency.one");
						} else {
							mutablecomponent1 = Component
									.translatable("randomizedminecraftmod.container.enchant.currency.many", i1);
						}
