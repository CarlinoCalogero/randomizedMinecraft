...
public class CurrencyEnchantmentMenu extends AbstractContainerMenu {
	private final Container enchantSlots = new SimpleContainer(2) {
		public void setChanged() {
			super.setChanged();
			CurrencyEnchantmentMenu.this.slotsChanged(this);
		}
	};
	...
	public CurrencyEnchantmentMenu(int p_39454_, Inventory p_39455_) {
		this(p_39454_, p_39455_, ContainerLevelAccess.NULL);
	}

	public CurrencyEnchantmentMenu(int p_39532_, Inventory p_39533_, FriendlyByteBuf extraData) {
		this(p_39532_, p_39533_, ContainerLevelAccess.NULL);
	}

	public CurrencyEnchantmentMenu(int p_39457_, Inventory p_39458_, ContainerLevelAccess p_39459_) {
		...
		if (p_39461_ == this.enchantSlots) {
			...
			if (!itemstack.isEmpty() && itemstack.isEnchantable()) {
				this.access.execute((p_39485_, p_39486_) -> {
					...
					for (BlockPos blockpos : CurrencyEnchantmentTableBlock.BOOKSHELF_OFFSETS) {
						if (CurrencyEnchantmentTableBlock.isValidBookShelf(p_39485_, p_39486_, blockpos)) {
							j += p_39485_.getBlockState(p_39486_.offset(blockpos)).getEnchantPowerBonus(p_39485_,
									p_39486_.offset(blockpos));
						}
					}
					...
					for (int k = 0; k < 3; ++k) {
						...
						this.costs[k] = net.minecraftforge.event.ForgeEventFactory.onEnchantmentLevelSet(p_39485_,
								p_39486_, k, (int) j, itemstack, costs[k]);
					}
					...
					for (int l = 0; l < 3; ++l) {
						if (this.costs[l] > 0) {
							...
							if (list != null && !list.isEmpty()) {
								...
								this.enchantClue[l] = BuiltInRegistries.ENCHANTMENT
										.getId(enchantmentinstance.enchantment);
	...
	public boolean clickMenuButton(Player p_39465_, int p_39466_) {
		if (p_39466_ >= 0 && p_39466_ < this.costs.length) {
			...
			if ((itemstack1.isEmpty() || itemstack1.getCount() < i) && !p_39465_.getAbilities().instabuild) {
				return false;
			} else if (this.costs[p_39466_] <= 0 || itemstack.isEmpty()
					|| (p_39465_.experienceLevel < i || p_39465_.experienceLevel < this.costs[p_39466_])
							&& !p_39465_.getAbilities().instabuild) {
				return false;
			} else {
				this.access.execute((p_39481_, p_39482_) -> {
					...
					if (!list.isEmpty()) {
						...
						p_39481_.playSound((Player) null, p_39482_, SoundEvents.ENCHANTMENT_TABLE_USE,
								SoundSource.BLOCKS, 1.0F, p_39481_.random.nextFloat() * 0.1F + 0.9F);
					}
	...
	public boolean stillValid(Player p_39463_) {
		return stillValid(this.access, p_39463_, ModBlocks.CURRENCY_ENCHANTING_TABLE.get());
	}
